...
int ac_tlm_filter::sobel_filter(int tl, int tc, int tr,
                                int ml, int mc, int mr,
                                int bl, int bc, int br) {
  int sum, sum_x = 0, sum_y = 0;

  // Horizontal component
  sum_x += 1 * tl + 0 * tc - 1 * tr;
  sum_x += 2 * ml + 0 * mc - 2 * mr;
  sum_x += 1 * bl + 0 * bc - 1 * br;

  // Vertical component
  sum_y +=  1 * tl + 2 * tc + 1 * tr;
  sum_y +=  0 * ml + 0 * mc + 0 * mr;
  sum_y += -1 * bl - 2 * bc - 1 * br;

  // Combine both
  sum = (int)sqrt(sum_x * sum_x + sum_y * sum_y);
